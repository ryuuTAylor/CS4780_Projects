...
def visualize_knn_images(findknn, imageType='faces'):
    ...
    def onclick(event):
        ...
        for i in range(4):
            ...
            for j in range(3):
                plt.subplot(4, 4, sb)
                plotimage(xdim, ydim, xTr[indices[j], :].reshape(
                    ydim, xdim).T, int(yTr[indices[j]] == yTe[who]))
                plt.axis('off')
                sb += 1
            who += 1
        plt.subplot(4, 4, 1)
...
# plot an image and draw a red/blue/green box around it (specified by "d")
def plotimage(xdim, ydim, M, d=2):
...
def visualize_knn_boundary(knnclassifier):
    ...
    def visboundary(k, Xdata, ldata, ax, knnclassifier):
        ...
        Z = Z.reshape(X.shape)
        boundary = ax.contourf(X, Y, np.sign(Z), colors=[
                               [0.5, 0.5, 1], [1, 0.5, 0.5]])
        plt.draw()
    ...
    cid = fig.canvas.mpl_connect('button_press_event', lambda event: onclickkdemo(
        event, Xdata, ldata, ax, knnclassifier))
